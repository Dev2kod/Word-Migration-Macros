...
function optimumDiameter() {
    ...
    // same algorithm as water supply: select Diameter corresponding to min Total Net Present Value
    let bestD = null;
    let bestZ = Number.POSITIVE_INFINITY;
    for (let i = 0; i < dtDatas2.length; ++i) {
        const zRaw = dtDatas2[i]['Total Net Present Value'];
        const dRaw = dtDatas[i]['Diameter'];
        const z = parseFloat(String(zRaw).replace(/,/g, ''));
        const d = dRaw;
        if (!Number.isFinite(z)) continue;
        if (z < bestZ) { bestZ = z; bestD = d; }
    }
    const out = byId('txtdia_opt'); if (out) out.value = bestD ?? '';
}
